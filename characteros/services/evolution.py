...
class EvolutionEngine:
    ...
    # 情緒映射表（emotion → AU 強度）
    EMOTION_AU_MAP = {
        "neutral": {"au_intensity": 0.1, "micro_expressions": ["relaxed"]},
        "happy": {"au_intensity": 0.6, "micro_expressions": ["smile_corners", "crow_feet"]},
        "sad": {"au_intensity": 0.4, "micro_expressions": ["downturned_mouth", "inner_brow_raise"]},
        "angry": {"au_intensity": 0.8, "micro_expressions": ["furrowed_brow", "tightened_lips"]},
        "fearful": {"au_intensity": 0.7, "micro_expressions": ["wide_eyes", "raised_brows"]},
        "determined": {"au_intensity": 0.5, "micro_expressions": ["focused_gaze", "slight_frown"]}
    }
    
    # 場景映射表（scene → 環境效果）
    SCENE_EFFECTS_MAP = {
        "battle": {"injury_chance": 0.8, "dirt_level": 0.6, "outfit_damage": "moderate"},
        "formal_event": {"injury_chance": 0.0, "dirt_level": 0.0, "outfit_damage": "none"},
        "casual_street": {"injury_chance": 0.1, "dirt_level": 0.2, "outfit_damage": "light_wear"},
        "post_apocalyptic": {"injury_chance": 0.6, "dirt_level": 0.8, "outfit_damage": "heavy"}
    }
    ...
    def _apply_emotion_evolution(self, manifest: Dict[str, Any], emotion: str) -> Dict[str, Any]:
        """應用情緒演化"""
        emotion_data = self.EMOTION_AU_MAP.get(emotion, self.EMOTION_AU_MAP["neutral"])
        
        if '_expression' in manifest:
            manifest['_expression']['base_emotion'] = emotion
    ...
    def _apply_scene_evolution(self, manifest: Dict[str, Any], scene: str) -> Dict[str, Any]:
        """應用場景演化"""
        scene_data = self.SCENE_EFFECTS_MAP.get(scene, {
            "injury_chance": 0.1,
            "dirt_level": 0.1,
            "outfit_damage": "none"
        })
    ...
    def _merge_custom_params(
        ...
        """合併自訂參數"""
        # 直接合併到頂層（避免覆蓋保留鍵）
        reserved_keys = ['_identity', '_style', '_expression', '_body', '_scene_context']
