...
def _task_item_from_dict(raw: dict, storage_mode: str) -> QueueTaskItem:
    ...
    evolution_params = copy.deepcopy(
        raw.get("evolution_params") if isinstance(raw.get("evolution_params"), dict) else {}
    )
    image_request = evolution_params.get("_image_request")
    if isinstance(image_request, dict) and "api_key" in image_request:
        image_request["has_api_key"] = bool(str(image_request.get("api_key") or "").strip())
        image_request.pop("api_key", None)
...
def get_system_metrics(db: Session = Depends(get_db)):
    ...
    if not is_database_available():
        ...
        return SystemMetricsResponse(
            ...
            total_variants=(
                qstats["total_pending"] + qstats.get("total_waiting", 0) + qstats.get("total_running", 0) + qstats["total_ready"] + qstats["total_failed"]
            ),
        )
